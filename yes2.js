...
class YesCoinBot {
    ...
    async useSpecialBox(token, proxy) {
        ...
        try {
            ...
            if (response.code === 0) {
                await this.log('Activating chest...', 'success');
                return true;
            } else {
        ...
        try {
            ...
            if (response.code === 0) {
                if (response.data.collectStatus) {
                    await this.log(`Open chest to receive ${response.data.collectAmount} Coins`, 'success');
    ...
    async attemptCollectSpecialBox(token, boxType, initialCoinCount, proxy) {
        ...
        await this.log('Unable to collect chest!', 'error');
        return 0;
    }
    ...
    async getTaskList(token, proxy) {
        ...
        try {
            ...
            if (response.code === 0) {
                ...
                await this.log(`Failed to get task list: ${response.message}`, 'error');
                return null;
            }
        } catch (error) {
    ...
    async finishTask(token, taskId, proxy) {
        ...
        try {
            ...
            if (response.code === 0) {
                await this.log(`Task ${taskId} successful | Reward: ${response.data.bonusAmount}`, 'success');
                return true;
            } else {
            ...
            await this.log(`Error while doing task: ${error.message}`, 'error');
            return false;
        }
    }
    ...
    async upgradeLevel(token, currentLevel, targetLevel, upgradeType, proxy) {
        ...
        if (currentLevel === targetLevel) {
            await this.log(`${upgradeTypeName} is already at ${currentLevel}`, 'info');
        }
    }
            ...
            if (accountBuildInfo.code === 0) {
                ...
                if (swipeBotLevel < 1) {
                    ...
                    if (upgradeResponse.code === 0) {
                        ...
                        await this.log('SwipeBot purchase failed', 'error');
                    }
                }
                ...
                if (swipeBotLevel >= 1 && !openSwipeBot) {
                    ...
                    if (toggleResponse.code === 0) {
                        await this.log('Successfully enabled SwipeBot', 'success');
                    } else {
                ...
                if (swipeBotLevel >= 1 && openSwipeBot) {
                    ...
                    if (offlineBonusInfo.code === 0 && offlineBonusInfo.data.length > 0) {
                        ...
                        if (claimResponse.code === 0) {
                            await this.log(`Claim offline bonus successful, receive ${claimResponse.data.collectAmount} coins`, 'success');
                        } else {
                ...
                await this.log('Failed to get SwipeBot information', 'error');
            }
        } catch (error) {
    ...
    async performTaskWithTimeout(task, taskName, timeoutMs = this.timeout) {
        return new Promise(async (resolve, reject) => {
            const timeoutId = setTimeout(() => {
                reject(new Error(`${taskName} timed out in ${timeoutMs}ms`));
            }, timeoutMs);
    ...
    async main() {
        try {
            ...
            try {
                ...
                await this.log(`Failed to get token: ${error.message}`, 'error');
                return;
            }
    ...
    try {
        ...
        if (bonusInfo.commonTaskBonusStatus === 1) {
            ...
            if (claimResponse.code === 0) {
            await this.log(`Claim Common Task bonus successful | reward ${claimResponse.data.bonusAmount}`, 'success');
            }
        }
        ...
        if (bonusInfo.dailyTaskBonusStatus === 1) {
            ...
            if (claimResponse.code === 0) {
            await this.log(`Claim Daily Task bonus successful | reward ${claimResponse.data.bonusAmount}`, 'success');
            }
        }
        ...
        await this.log(`Error while checking and claiming Task bonus: ${error.message}`, 'error');
        return false;
    }
    }
	...
	async performDailyMissions(token, proxy) {
		try {
			...
			if (dailyMissionsResponse.code === 0) {
					...
					if (mission.missionStatus === 0) {
						...
						if (checkResponse.code === 0 && checkResponse.data === true) {
							...
							if (claimResponse.code === 0) {
								...
								await this.log(`Mission ${mission.name} successful | Reward: ${reward}`, 'success');
							} else {
								await this.log(`Claiming mission ${mission.name} reward failed: ${claimResponse.message}`, 'error');
							}
						} else {
							await this.log(`Checking mission ${mission.name} failed`, 'error');
						}
				...
				await this.log(`Failed to get daily tasks list: ${dailyMissionsResponse.message}`, 'error');
				return false;
			}
		} catch (error) {
			await this.log(`Error while executing daily task: ${error.message}`, 'error');
			return false;
		}
	}
        ...
        try {
            ...
            if (signInListResponse.code === 0) {
                ...
                if (availableSignIn) {
                    ...
                    // Full header for roll call request
                    const headers = {
                        'Accept': 'application/json, text/plain, */*',
                        'Accept-Language': 'en-US,en;q=0.9',
                        'Cache-Control': 'no-cache',
                    ...
                    if (signInResponse.code === 0) {
                        ...
                        await this.log(`Daily check-in successful | Reward: ${reward}`, 'success');
                        return true;
                    } else {
                        await this.log(`Daily roll call failed: ${JSON.stringify({
                    ...
                    await this.log(`You have checked in today`, 'warning');
                    return false;
                }
            } else {
                await this.log(`Could not get attendance list: ${JSON.stringify({
            ...
            await this.log(`Error while performing daily roll call: ${error.message}
            Stack: ${error.stack}
    ...
    async performTasks() {
        try {
            ...
            if (squadInfo && squadInfo.data.isJoinSquad) {
                ...
                await this.log('Squad: You are not in a Squad, join the Airdrop Farmer.', 'warning');
                ...
                if (joinResult) {
                    await this.log(`Squad: ${nickname} joined Squad successfully!`, 'success');
                } else {
                ...
                if (recoverResult) {
                    ...
                    if (updatedCollectInfo) {
                        ...
                        if (coinPoolLeftCount > 0) {
                            ...
                            if (collectResult && collectResult.code === 0) {
                                const collectedAmount = collectResult.data.collectAmount;
                                await this.log(`Tap successfully after recovery, received ${collectedAmount} coins`, 'success');
                            } else {
...
if (isMainThread) {
    ...
    async function processCycle() {
        console.log("If you use it, don't be afraid, if you're afraid, don't use it...".magenta);
        let accountQueue = [...accounts];

        function startWorker() {
            if (accountQueue.length === 0) {
                if (activeWorkers === 0) {
                    console.log('Finished all accounts, take a break.'.green);
